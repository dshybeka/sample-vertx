...
public class AuditVerticle extends RxMicroServiceVerticle {
    ...
    public void start(Future<Void> future) {
        ...
        // TODO
        // ----
        Single<Void> initDbDingle = initializeDatabase(config().getBoolean("drop", false));
        Single<Void> initHttpSingle = configureTheHTTPServer().flatMap(server -> rxPublishHttpEndpoint("audit", "localhost", server.actualPort()));
        Single<MessageConsumer<JsonObject>> portfolioSingle = retrieveThePortfolioMessageSource();

        Single<MessageConsumer<JsonObject>> readySingle = Single.zip(initDbDingle, initHttpSingle, portfolioSingle, (db, http, consumer) -> consumer);
        // ----
    ...
    private void retrieveOperations(RoutingContext context) {
        ...
        jdbc.rxGetConnection()
                .flatMap(sqlConnection -> sqlConnection.rxCall(SELECT_STATEMENT)).map(resultSet -> resultSet.getRows().stream().map(row -> new JsonObject(row.getString("OPERATION")))
                .collect(Collectors.toList())
        ).subscribe(data -> {

            System.out.println("return following data " + Json.encode(data));

            context.response().putHeader("content-type", "application/json");
            context.response().setStatusCode(200).end(Json.encode(data));
        }, throwable -> {
    ...
    private Single<HttpServer> configureTheHTTPServer() {
        ...
        Single<HttpServer> httpServerSingle = vertx.createHttpServer()
